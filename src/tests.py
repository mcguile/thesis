...
def test_comms(folder, intention_criteria, full_swarm, inf_moves, num_games, player1, player2):
    ...
    for i in range(1, num_games+1):
        ...
        game = GameNoUI(state=g,
                        ...
                        log_file=None)#f'{folder}{i}_{player1}_{player2}_{intention_criteria}.txt')
        winner = game.play_full_game()
        if winner == -1:
            if game.state.turn_count_white > (11 + rand_moves):
                # Accounting for random moves
...
class SwarmingTest(unittest.TestCase):
    ...
    #     test_comms(folder='logs/velocity/', intention_criteria=0, full_swarm=False, inf_moves=False, num_games=300,
    #                player1=player_swarm, player2=player_random)

    # def test_swarm1_acc(self):
    #     print('\nTesting accuracy with DT')
    #     test_comms(folder='logs/accuracy/', intention_criteria=1, full_swarm=False, inf_moves=False, num_games=300,
    #                player1=player_swarm, player2=player_random)
    #
    # def test_swarm2_fitness(self):
    #     print('\nTesting fitness with DT')
    #     test_comms(folder='logs/fitness/', intention_criteria=2, full_swarm=False, inf_moves=False, num_games=300,
    #                player1=player_swarm, player2=player_random)
    #
    # def test_swarm3_vel_and_acc(self):
    #     print('\nTesting velocity + accuracy with DT')
    #     test_comms(folder='logs/vel_and_acc/', intention_criteria=3, full_swarm=False, inf_moves=False, num_games=300,
    #                player1=player_swarm, player2=player_random)
    # def test_swarm4_vel_or_acc(self):
    #     print('\nTesting velocity or accuracy with DT')
    #     test_comms(folder='logs/vel_or_acc/', intention_criteria=4, full_swarm=False, inf_moves=False, num_games=300,
    #                player1=player_swarm, player2=player_random)
    #
    def test_swarm5_vel_or_dgt(self):
        print('\nTesting velocity or DGT with DT')
        test_comms(folder='logs/vel_or_dgt/', intention_criteria=5, full_swarm=False, inf_moves=False, num_games=100,
                   player1=player_swarm, player2=player_random)
